...
>
> **Historical validation record:** this work produced the D-008 pivot and was superseded by D-009 before that framing shipped. The runtime that ships is self-contained and host-agnostic, with no Honcho or Graphiti dependency. The current component map is [Runtime Architecture](./RUNTIME-ARCHITECTURE.md). The plan below is preserved as the evidence that produced those decisions.
...
2. **The unflagged killer risk is resurrection confabulation.** A model summarizes a model-generated conversation; another model reconstructs the "prior self" from that summary. The measured failure mode (see [`research/findings/03`](../research/findings/03-crp-reconstruction.md)) is *silent information loss* + *no internal error signal* (calibration ECE 0.45–0.75) + history-size degradation (30–64%), with no ground-truth validation specified anywhere in the RFC.
3. **The epistemic graph is both the differentiator and the bomb.** It is the only feature neither Letta nor Mem0 has shipped — but raw NLI contradiction detection scores just **23.94% precision on natural dialogue** ([DECODE](https://arxiv.org/abs/2012.13391)). The viable path is a temporal-KG pipeline ([`research/findings/04`](../research/findings/04-epistemic-graph.md)), which Track C now tests. Crown jewel and most-likely-to-feel-broken are the same feature. Honcho may already cover part of it.
...
> **Revised per research finding D-005.** The original protocol tested raw "compare two belief sets, find contradictions" — i.e. raw NLI-style detection. The literature already answers that: raw NLI scores **23.94% precision on natural dialogue** ([DECODE](https://arxiv.org/abs/2012.13391)). Re-measuring it would just re-confirm a known floor. So Track C now tests the **architecture we'd actually ship** ([`research/findings/04`](../research/findings/04-epistemic-graph.md)): Claimify-style extraction → temporal-KG validity intervals → flag a contradiction **only** on interval overlap. Raw NLI is kept as a documented baseline arm to prove the lift.
