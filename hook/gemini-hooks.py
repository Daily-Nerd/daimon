#!/usr/bin/env python3
"""Daimon Gemini CLI hook lifecycle manager.

Installs two Gemini CLI command hooks:
  - SessionStart -> inject latest daimon briefing (hookSpecificOutput JSON)
  - SessionEnd   -> serialize Gemini transcript (detached; currently a graceful
                    no-op while upstream transcript_path is a stub, gemini-cli#14715)

Gemini discovers hooks from settings.json (user layer: ~/.gemini/settings.json;
docs/hooks: index.md "Configuration", verified 2026-07-01). Schema per event:
an array of hook definitions {matcher?, sequential?, hooks: [{type: "command",
command, name?, timeout?, description?}]}. Two host-specific traps:
  - `timeout` is in MILLISECONDS (default 60000), not seconds.
  - lifecycle matchers are EXACT strings ("startup"), not regexes — a
    Codex-style "startup|resume" matcher would never fire. We omit the matcher
    entirely (matches all sources), because briefing after /clear is desirable:
    the context was just wiped.
"""
...
HOOKS = [
    {
        ...
        "entry": {
            "hooks": [{
                "type": "command",
                "command": "python3 ~/.gemini/hooks/daimon-gemini-session-start.py",
                "name": "daimon-brief",
                "timeout": 10000,  # milliseconds
                "description": "Inject the latest daimon checkpoint briefing",
            }],
        },
    },
    {
        "script": "daimon-gemini-session-end.py",
        "event": "SessionEnd",
        "entry": {
            "hooks": [{
                "type": "command",
                "command": "python3 ~/.gemini/hooks/daimon-gemini-session-end.py",
                "name": "daimon-checkpoint",
                "timeout": 10000,  # milliseconds
                "description": "Serialize the ending session into a daimon checkpoint",
            }],
        },
    },
]
...
def save_settings(settings, dry):
    ...
    if SETTINGS.exists():
        ...
        print(f"  settings.json backup: {backup.name}")
...
def install(dry):
    ...
    print("install: done" + (" (dry-run, nothing written)" if dry else
          ". Open /hooks panel in Gemini CLI to review the hook definitions."))
...
def status():
    ...
    for spec in HOOKS:
        ...
        print(f"{spec['script']:34} {spec['event']:13} {state}{extra}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Daimon Gemini CLI hook lifecycle manager.")
