...
| `daimon hooks install [host]` | Ship the host hook scripts (Windsurf, Codex, Kimi) from the package. With no host, daimon detects what this machine runs, prints one line per host with the channel that serves it, and installs: silently for a single host, after one confirmation for several, and never without a terminal unless `--all` is passed. Also re-arms every ruling layer above this project, so a check ratified in a layer above it arms here too, even on a machine that has never synced that layer directly; a layer line prints only when the layer actually armed something. `remove [host]` unregisters what daimon registered, and `--all` covers every detected host. `list` / `status` inspect; `status` also audits this project's check manifest against its ledger, and exits non-zero on either kind of drift. |
...
| `daimon skill status` | Audit each installed skill against what this CLI would write now: CURRENT, STALE, MISSING, BROKEN, NOT INSTALLED, PLUGIN or LEFTOVER, per host and scope, for both the generated `daimon` skill and the bundled `daimon-end`. Non-zero exit on drift, `--json` for machines. The skill is a static copy of content that moves with the CLI, so this is how you find out that a re-run after an upgrade was missed. A host served by the daimon plugin reports PLUGIN and needs nothing; a file left behind under one reports LEFTOVER, and its repair is `uninstall`, not `install`. |
| `daimon check sync` | Rebuild `~/.daimon/checks` — the manifest of armed checks a host hook reads, and one file per check body — from this project's ledger, and from every ruling layer above it, so a layer's check re-arms here without a human ever running this command from inside the layer's own project on this machine. Every ratify, revise, retire and forget already does this, so run it only when the manifest was damaged out of band. Safe to repeat: an unchanged manifest is not rewritten. Prints how many checks are armed, zero included, with one further line per layer that armed something or failed to sync. `--check` audits instead of rebuilding and writes nothing, grading every layer above this project the same way: exit 0 in step, 1 drifted (this project's manifest, or a layer's), 3 when a manifest exists and cannot be read. `--json` for machines, with a `layers` key present only when a layer contributed something. |
...
| `daimon diff` | What changed between two retained checkpoints of this project: items added, restated, re-tagged, and every item the chain stopped carrying with the reason the record gives (resolved, superseded, forgotten, or dropped when nothing says). Defaults to the latest readable pair; `--from N --to M` counts generations back, where 0 is `latest.json`. `--json` for rows. Read-only, and there is no restore: a checkpoint records what a session did, so restoring one would assert a different past. Say something changed with `resolve`, `reverify` or `forget` instead. |
| `daimon blame <item-id>` | How one item got here: the session that first stated it, every carry since, and every event that changed its state, in order. `daimon why` answers where a claim came from; this answers how it got here. A forgotten item keeps its tombstone and never its text. `--json` for machines. |
...
| `daimon audit quotes` | Re-check every stored verbatim quote against its source transcript and report mismatches. Read-only — it never rewrites trust tags. Exit 0 checked clean, 1 mismatch, 3 nothing checkable. It always reports how many items it had to skip, and when that is all of them it says so instead of printing a rate over nothing. `--json` for machines, and it lists every failure, where the printed lines stop at `--top`. |
| `daimon audit privacy` | Prove the deletion contract: hash every plaintext field on every surface (checkpoints, rotated pointers, the event ledger, the team mirror, the recall index and its orphan snapshots) and report any forgotten value that survived. Read-only. A value only a teammate forgot that still sits in plaintext here is reported as `SUPPRESSED-PRESENT`, and this class never changes the exit code, since scrubbing it is opt-in through `DAIMON_TEAM_APPLY_FORGET`. |
...
| `daimon ruling list\|show` | Read the standing rulings: human-ratified positive constraints on the same ledger, never decayed, never re-extracted. `show` includes pending agent proposals, and resolves across a ruling layer above this project too, so an inherited id shows the ruling and names its layer instead of reading unknown. `list` stays scoped to this project's own bucket unless `--inherited` is passed, which adds the active rulings inherited from a layer above it (each `--json` row carries `inherited_from`); refused under `DAIMON_TENANT_SCOPED`, the same as every other caller-chosen cross-project scope. A `list` that finds nothing exits 1 and names the bucket on stderr when the project has never been written from, and exits 0 when the project has a bucket that holds no rulings. |
| `daimon ruling checks` | What this project has armed: one row per ruling that carries a check, crossed with every host, including a layer's active check inherited from above this project (labelled with the layer). Each row names the check's lifecycle, the intent its author asked for, the mode that host actually delivers, when it last ran, and its clean / violation / unresolved counts over the window the log still holds. A row that could not have run ends after its mode, because nothing is armed for a proposed or disarmed check and a host that cannot deliver one has no channel to run it through. Header lines report the manifest state, the firing log's own state, the timestamp the retained window starts at, and every host the hook has ever run on. Those host lines are labelled `(any project)`: the rows behind them carry no project, so hook liveness is a fact about this machine, never about this project. Read-only, and an empty answer exits 0. `--json` for machines. |
| `daimon ruling check try <id> --command "<cmd>"` | Run this ruling's check against a command you name and print the outcome. Resolves an id inherited from a layer above this project too, and names the layer in that case. Arms nothing, logs nothing, and materializes the body outside the checks directory. `--cwd` sets the working directory relative file arguments resolve against; `--proposed` runs a pending agent revision's body instead of the armed one. Human path only, like `ratify`: it executes the body, so an interactive terminal is required and `--by agent` is refused. Same exit contract as the auditors below. |
...
| `daimon ruling propose\|ratify\|revise\|retire` | Manage standing rulings on the same ledger, with a stricter lifecycle: `ratify` shows the full text, discloses that it will render into every future session, and binds the activation to the text it displayed; a human revising an active ruling confirms the change and the ruling stays active; agent revise and retire calls record proposals while the text stands; activation refuses past the cap (`DAIMON_RULING_CAP`, default 7). Retirement needs no evidence citation. `propose` and `revise` may attach a check with `--check-body-file`, `--check-match` and `--check-intent`: a script whose bytes are stored on the ruling (a path is refused), a pattern on the command string that selects the actions it runs before, and what it asks each host for (`warn` by default). A candidate's check reads as proposed, not armed, and no host runs it. `ratify` prints the check and binds the activation to the body it showed, by hash, and materializes it for a host to run. A body that names a host-local path on any line is refused, because the check travels with the ruling and a path outside it is missing on every other machine. See [Checks at runtime](#checks-at-runtime). `propose` and `revise` may also carry a `--request-policy` grant, in one of two shapes (repeatable, all four keys required, verb-branched, and mixing the two is refused): `--request-policy sender=<slug> kind=work verb=accept by=agent` lets that sender project's agent record `accept` on a `work` request this project owes it; `--request-policy to=<slug> kind=info verb=open by=agent` lets THIS project's own agent open a request to that recipient as `kind=info`, with no person touching it (no wildcard `to`). `ratify` pins the activation to the grant it showed the same way it pins a check, and `revise --no-request-policy` clears it. |
| `daimon amend propose <item-id> --change progressed\|blocked\|changed --evidence "<quote>"` | Record an evidence-carrying state transition on an open briefed item: the loop stays open, its state moved. `--evidence` is a verbatim transcript quote, byte-checked against this session's transcript at session end, not resolved or verified beyond that. An agent's proposal stays invisible in the briefing until the byte-check passes, then renders flagged and unconfirmed with its own confirm/reject pair; only a human `ratify` renders it as settled. `ratify`/`reject` take one or more ids, all-or-nothing if any is unknown or in the wrong state. `list` shows every amendment for this project, candidates first. |

### Layer rulings

A ruling proposed and ratified at a plain directory above your repositories binds every project beneath it: run `daimon ruling propose` with the rule text and its usual flags, then `daimon ruling ratify <id> --project ~/work` from an interactive terminal, and it arms there and for everything underneath. A layer is any such directory at or below your home directory that carries no `.git` anywhere at or above it and already holds a daimon bucket.

A child project's briefing renders these inherited rulings first, each one tagged `[from ~/work]`, and they count against the child's own cap of 7: the ceremony at the layer warns which of its descendants an activation would push over that cap. `retire`, `revise` and `ratify` on an inherited id refuse inside the child and point at `--project ~/work` instead, since only the layer's own bucket can act on its ruling. `daimon ruling list --inherited` adds the merged view to a project's own list, and each row it adds carries `inherited_from`, in `--json` too. A check attached to a layer ruling arms in every repository below it the next time `daimon hooks install` or `daimon check sync` runs there.
...
- These two readers now differ. `listing` stays scoped to this project's own bucket, and only its rows feed the check manifest a host hook reads, so a check never arms twice under the same project. `active_rulings` is the merged view a briefing renders: this project's own rows plus every active ruling from a layer above it, layers first, own wins on a shared id, an inherited request-for-a-policy row dropped rather than rendered, and each layer row tagged `inherited_from` with the absolute directory it came from. `daimon ruling list --inherited` is the CLI form of that merged read: without the flag it stays own-bucket only, and with it the layer rows are appended after this project's own, each one carrying `inherited_from` in `--json` too; a project's own row never carries that key at all.
- `daimon_briefing.briefing.rulings_read(project_dir)` is the sibling a host reads when an empty answer is not enough: `active_rulings` always returns a plain list and folds every failure into an empty one, so a project that was never identified, a mis-resolved path, an unreadable ledger, and a project with genuinely no rulings all read exactly the same. `rulings_read` never raises and always returns one of four `state` values alongside the same `rows` and the `path` it resolved to: `"unresolved"` (the ledger path itself could not be worked out at all — a config problem, `path` is None here and only here), `"no-bucket"` (the path resolved, but nothing has ever been written from this project), `"unreadable"` (the bucket exists but the ledger could not be read), or `"read"` (a normal read, including a clean empty bucket). `active_rulings` is built on top of it, never raises either, and cannot drift from it.
...
| `daimon request accept\|reject\|needs-info <id> [--note]` | Land a decision. Human-only — requires an interactive terminal. An agent may run `accept --by agent` directly for an addressed `info` ask still open or needs-info, and the record says so; a `work` ask still needs a person, unless an active standing ruling on this project grants that specific sender an accept, in which case the record names the ruling too. `reject` is final for that record; the sender supersedes with a new request rather than asking again. |
...
Only a human channel may open a request as `kind=info`. An agent may do so only under an active `verb=open` ruling its own project ratified for that recipient; the record then reads `Kind: info (opened by agent under r-...)` on `list`, `show` and `inbox`, so a ruling-classified ask never looks like one a person opened.
...
| command | what it does |
| --- | --- |
| `daimon status` | Checkpoint presence and age, last serialize outcome, health warnings. One line reports how many checks this project has armed, counting an active check inherited from a layer above it too, how many are still proposed, and how long ago one last ran, plus a pointer to `daimon check sync` when the manifest has drifted. It appears only when this project has a check at all, and never moves the exit code. `--suppressed` lists withheld resolved items. |
| `daimon stats` | Local usage and capture aggregates — nothing is transmitted; sharing the output is a deliberate paste. Includes a receipt-probe line with lifetime totals (attempted, eligible, confirmed, contradicted, skipped, cured) when receipts are configured. Also one checks line in three wordings: nothing armed, armed but never fired, or totals (fired, clean, violation, unresolved, denied) aggregated across every host and over the window the capped firing log still holds, which the line names; the armed count includes an active check inherited from a layer above this project. `--json` for machines. |
...
| `daimon bucket migrate` | Move the bucket a pre-0.42.0 daimon wrote for this project into the one this daimon reads. Two path shapes are affected: a symlink anywhere in the path, and a subdirectory of a git repository written through the library rather than the CLI. Renames when the new bucket does not exist yet, merges line by line when it does. A pointer already in the new bucket is never removed or displaced; old pointers fill the free slots. Safe to run twice. Exit 1 means the move did not finish and names each thing left with its remedy; exit 2 refuses a `--project` containing `..`, and refuses `--project` at all on a tenant-scoped home. `--dry-run` prints the plan and writes nothing; `--json` prints the record it appends to `~/.daimon/checkpoints/migrations.jsonl`, or `{"refused": "..."}`. |
...
| `daimon action-recall` | The pre-shell-action recall backend: a shell command on stdin, zero or one prior-work line out, exit 0 always. A separate process from the pre-action check that can deny a command, so a fault here never drops a deny. |
| `daimon request-inject` | The per-prompt live-delivery backend: prints undecided cross-project asks this session has not already been shown, exit 0 always. |
