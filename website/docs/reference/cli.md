...
| `daimon bucket migrate` | Move the bucket a pre-0.42.0 daimon wrote for this project into the one this daimon reads. Two path shapes are affected: a symlink anywhere in the path, and a subdirectory of a git repository written through the library rather than the CLI. Renames when the new bucket does not exist yet, merges line by line when it does, and leaves anything it does not recognize in place and reports it. Safe to run twice. Exit 2 refuses a `--project` path containing `..`, which the two rules would resolve to different directories; exit 1 means a file could not be read and was left where it is. `--dry-run` prints the plan and writes nothing; `--json` prints the record it appends to `~/.daimon/checkpoints/migrations.jsonl`. |
