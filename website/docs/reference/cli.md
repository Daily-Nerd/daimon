...
| `daimon skill status` | Audit each installed skill against what this CLI would write now: CURRENT, STALE, MISSING, BROKEN or NOT INSTALLED, per host and scope, for both the generated `daimon` skill and the bundled `daimon-end`. Non-zero exit on drift, `--json` for machines. The skill is a static copy of content that moves with the CLI, so this is how you find out that a re-run after an upgrade was missed. |
