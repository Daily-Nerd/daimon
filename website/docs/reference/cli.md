...
`project_dir` is resolved the same way the CLI resolves `--project`: made absolute, symlinks collapsed, then normalized to the git toplevel. A host standing in a subdirectory of a repository therefore reads and writes the same bucket the CLI reads from the repository root. `daimon_briefing.config.resolve_project_dir(path)` is the public function that returns the directory a path routes to; a value with no path separator that names no existing directory is taken as a bucket slug and passes through untouched.
