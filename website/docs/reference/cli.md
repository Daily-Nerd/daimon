...
| `daimon ruling checks` | What this project has armed: one row per ruling that carries a check, crossed with every host. Each row names the check's lifecycle, the intent its author asked for, the mode that host actually delivers, when it last ran, and its lifetime clean / violation / unresolved counts. A row that could not have run ends after its mode, because nothing is armed for a proposed or disarmed check and a host that cannot deliver one has no channel to run it through. Header lines report the manifest state and every host the hook has ever run on. Read-only, and an empty answer exits 0. `--json` for machines. |
