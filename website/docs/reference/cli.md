...
- `daimon_briefing.briefing.rulings_read(project_dir)` is the sibling a host reads when an empty answer is not enough: `active_rulings` always returns a plain list and folds every failure into an empty one, so a mis-resolved path and an unreadable ledger both read exactly like a project with no rulings. `rulings_read` returns the same `rows` alongside a `state` naming which one happened — `"read"` (a normal read, including a clean empty bucket), `"no-bucket"` (nothing has ever been written from this project), or `"unreadable"` (the bucket exists but the ledger could not be read) — and the `path` it resolved to. `active_rulings` is built on top of it and cannot drift from it.
