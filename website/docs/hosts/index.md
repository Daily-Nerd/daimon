...
| [Gemini CLI](./gemini.md) | Manual `hook/gemini-hooks.py install` (from a clone) | `SessionEnd` hook spawns `daimon serialize` once `transcript_path` is non-empty (gemini-cli v0.21.0 or later; the `gemini-cli#14715` stub is fixed there) | `SessionStart` hook injects via `additionalContext` | capture unverified: no real Gemini session run through serialize since the upstream fix |
