...
| `DAIMON_BRIEF_MAX_BYTES` | `11264` | Tope duro sobre el briefing final ya renderizado, en bytes UTF-8, aplicado después de ensamblar cada sección (reglas vigentes, los paneles de solicitudes y decisiones, y el cuerpo cognitivo). Es un chequeo separado y posterior al `DAIMON_BRIEF_MAX_TOKENS` de arriba: el presupuesto de tokens nunca ve el armazón fijo, y los bytes son lo que realmente mide el límite de recorte de un host. `0` = sin límite. |
