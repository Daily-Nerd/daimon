...
| `DAIMON_BRIEF_MAX_BYTES` | `11264` | Tope duro sobre la salida COMPLETA de `daimon brief`, en bytes UTF-8: el bloque de traspaso (handoff), el cuerpo renderizado (reglas vigentes, los paneles de solicitudes y decisiones, y el cuerpo cognitivo), el bloque de desvío de código y la sección de compañeros de equipo juntos, no solo el cuerpo. Es un chequeo separado y posterior al `DAIMON_BRIEF_MAX_TOKENS` de arriba: el presupuesto de tokens nunca ve el armazón fijo, y los bytes son lo que realmente mide el límite de recorte de un host. `0` = sin límite. |
