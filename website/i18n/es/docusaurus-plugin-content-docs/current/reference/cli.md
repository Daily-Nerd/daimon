...
Tres superficies lo leen por vos. `daimon ruling checks` lo pliega por regla y por anfitrión, `daimon stats` reporta una línea acumulada sobre todos los anfitriones, y `daimon ruling show` agrega una línea `Fired:` a una sola regla. Las tres reportan una marca de tiempo y conteos acumulados, nunca el último resultado: un log que solo agrega está ordenado por el agregado, así que la última fila no es el estado actual. Un check armado sin ninguna fila se lee como `never fired`, que es una respuesta distinta de cero violaciones.
...
En las dos llamadas de arriba, y en los helpers de los ledgers de refutaciones, pedidos, enmiendas y relaciones que están detrás, `project_dir` se resuelve igual que la CLI resuelve `--project`: se vuelve absoluto, se colapsan los symlinks y después se normaliza al toplevel de git. Un anfitrión parado en un subdirectorio de un repositorio lee el mismo bucket que la CLI lee desde la raíz del repositorio. `daimon_briefing.config.resolve_project_dir(path)` es la función pública que devuelve el directorio al que rutea una ruta. Ojo con el alcance: esto cubre las superficies de ledger nombradas acá, no toda función que reciba un `project_dir`. El store de checkpoints sigue derivando su bucket de la ruta literal que le pasás, así que pasale un directorio ya resuelto si lo llamás por tu cuenta. La CLI resuelve antes de llamar a cualquiera de los dos, así que un checkpoint escrito por un hook y un ledger escrito por la CLI siempre coinciden.
...
| `daimon stats` | Agregados locales de uso y captura — nada se transmite; compartir la salida es un pegado deliberado. Incluye una línea de sondeos de recibo con totales acumulados (intentados, elegibles, confirmados, contradichos, omitidos, curados) cuando los recibos están configurados. También una línea de checks en tres redacciones: nada armado, armado pero nunca corrió, o totales acumulados (corridas, clean, violation, unresolved, denegadas) sumados sobre todos los anfitriones. `--json` para máquinas. |
